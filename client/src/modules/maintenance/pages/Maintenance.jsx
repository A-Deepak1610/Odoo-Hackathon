...
const Maintenance = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (isAdmin) {
        const assetsEndpoint = "/api/v1/assets";
        const ticketsEndpoint = "/api/v1/maintenance";
        ...
        if (assetsRes.ok && ticketsRes.ok) {
          ...
          if (loadedTickets.length === 0) {
            loadedTickets = MOCK_TICKETS_HISTORY;
          }
        ...
        if (!storedTickets) {
          storedTickets = JSON.stringify(MOCK_TICKETS_HISTORY);
